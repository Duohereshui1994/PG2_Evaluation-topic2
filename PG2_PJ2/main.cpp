...
const char kWindowTitle[] = "Flight_Fight";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (scene->getSceneNum() == 1)
		{
			...
			Novice::ScreenPrintf(10, 15, "score = %d", player->getScore());
			//hpbar
			...
			if (player->getHp() < player->getHpMax() / 2 && player->getHp() >= player->getHpMax() * 0.3f) {
				Novice::DrawBox(440, 670, (int)(player->getHp() / player->getHpMax() * 400), 30, 0.0f, 0xFFFF00FF, kFillModeSolid);
			}
			if (player->getHp() < player->getHpMax() * 0.3f && player->getHp() >= 0) {
				Novice::DrawBox(440, 670, (int)(player->getHp() / player->getHpMax() * 400), 30, 0.0f, RED, kFillModeSolid);
			}
		}
		///
		/// ↑描画処理ここまで
